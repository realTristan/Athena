...
class Settings(commands.Cog):
    ...
    async def _del_map(self, ctx, map):
        if await SQL.exists(f"SELECT * FROM maps WHERE guild_id = {ctx.guild.id} AND lobby_id = {ctx.channel.id}"):
            row = await SQL.select(f"SELECT * FROM maps WHERE guild_id = {ctx.guild.id} AND lobby_id = {ctx.channel.id}")
            map_list = str(row[2]).split(',')
            if map in map_list:
                map_list.remove(map)
                await SQL.execute(f"UPDATE maps SET map_list = '{','.join(str(e) for e in map_list)}' WHERE guild_id = {ctx.guild.id} AND lobby_id = {ctx.channel.id}")
    ...
    async def lobby(self, ctx, action:str):
        ...
        if row is None:
            await SQL.execute(f"INSERT INTO lobbies (guild_id, lobby_list) VALUES ({ctx.guild.id}, '{ctx.channel.id}')")
        ...
        if action in ["add", "create"]:
            ...
            if len(lobbies) < 4:
                if not str(ctx.channel.id) in lobbies:
                    await SQL.execute(f"UPDATE lobbies SET lobby_list = '{str(row[1])},{ctx.channel.id}' WHERE guild_id = {ctx.guild.id}")
                    if not await SQL.exists(f"SELECT * FROM lobby_settings WHERE guild_id = {ctx.guild.id} AND lobby_id = {ctx.channel.id}"):
                        await SQL.execute(f"INSERT INTO lobby_settings (guild_id, lobby_id, map_pick_phase, team_pick_phase, win_elo, loss_elo, party_size) VALUES ({ctx.guild.id}, {ctx.channel.id}, 'false', 'true', 5, 2, 1)")
            ...
            try:
                ...
                if res.values[0] == 'map_pick_phase':
                    if res.author.guild_permissions.administrator:
                        row = await SQL.select(f"SELECT * FROM lobby_settings WHERE guild_id = {res.guild.id} AND lobby_id = {res.channel.id}")
                        if row[2] == "false":
                            await SQL.execute(f"UPDATE lobby_settings SET map_pick_phase = 'true' WHERE guild_id = {res.guild.id} AND lobby_id = {res.channel.id}")
                        ...
                        await SQL.execute(f"UPDATE lobby_settings SET map_pick_phase = 'false' WHERE guild_id = {res.guild.id} AND lobby_id = {res.channel.id}")
                        await res.send(embed=discord.Embed(description=f"{res.author.mention} Success!", color=3066992))
                ...
                if res.values[0] == 'team_pick_phase':
                    if res.author.guild_permissions.administrator:
                        row = await SQL.select(f"SELECT * FROM settings WHERE guild_id = {res.guild.id} AND lobby_id = {res.channel.id}")
                        if row[3] == "false":
                            await SQL.execute(f"UPDATE lobby_settings SET team_pick_phase = 'true' WHERE guild_id = {res.guild.id} AND lobby_id = {res.channel.id}")
                            await res.send(embed=discord.Embed(description=f"{res.author.mention} Success!", color=3066992))
                        ...
                        await SQL.execute(f"UPDATE lobby_settings SET team_pick_phase = 'false' WHERE guild_id = {res.guild.id} AND lobby_id = {res.channel.id}")
                        await res.send(embed=discord.Embed(description=f"{res.author.mention} Success!", color=3066992))
                ...
                if res.values[0] == "change_win_elo":
                    if res.author.guild_permissions.administrator:
                        ...
                        await SQL.execute(f"UPDATE lobby_settings SET win_elo = {int(c.content)} WHERE guild_id = {res.guild.id} AND lobby_id = {res.channel.id}")
                ...
                if res.values[0] == "change_loss_elo":
                    if res.author.guild_permissions.administrator:
                        ...
                        await SQL.execute(f"UPDATE lobby_settings SET loss_elo = {int(c.content)} WHERE guild_id = {res.guild.id} AND lobby_id = {res.channel.id}")
                ...
                if res.values[0] == "change_queue_party_size":
                    if res.author.guild_permissions.administrator:
                        ...
                        await SQL.execute(f"UPDATE lobby_settings SET party_size = {int(c.content)} WHERE guild_id = {res.guild.id} AND lobby_id = {res.channel.id}")
