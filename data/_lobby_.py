...
class Lobby:
    ...
    async def update(guild_id: int, lobby_id: int, map_pick_phase=None, team_pick_phase=None, win_elo=None, loss_elo=None, party_size=None, negative_elo=None, queue_size=None):
        ...
        if map_pick_phase is not None:
            await Cache.update("lobbies", guild=guild_id, lobby=lobby_id, data={"map_pick_phase": map_pick_phase}, sqlcmds=[
        ...
        if team_pick_phase is not None:
            await Cache.update("lobbies", guild=guild_id, lobby=lobby_id, data={"team_pick_phase": team_pick_phase}, sqlcmds=[
                f"UPDATE lobbies SET team_pick_phase = {team_pick_phase} WHERE lobby_id = {lobby_id} AND guild_id = {guild_id}"
            ])
        
        # // Update win elo
        if win_elo is not None:
            await Cache.update("lobbies", guild=guild_id, lobby=lobby_id, data={"win_elo": win_elo}, sqlcmds=[
                f"UPDATE lobbies SET win_elo = {win_elo} WHERE lobby_id = {lobby_id} AND guild_id = {guild_id}"
            ])

        # // Update loss elo
        if loss_elo is not None:
            await Cache.update("lobbies", guild=guild_id, lobby=lobby_id, data={"loss_elo": loss_elo}, sqlcmds=[
                f"UPDATE lobbies SET loss_elo = {loss_elo} WHERE lobby_id = {lobby_id} AND guild_id = {guild_id}"
            ])

        # // Update party size
        if party_size is not None:
            await Cache.update("lobbies", guild=guild_id, lobby=lobby_id, data={"party_size": party_size}, sqlcmds=[
                f"UPDATE lobbies SET party_size = {party_size} WHERE lobby_id = {lobby_id} AND guild_id = {guild_id}"
            ])

        # // Update negative elo
        if negative_elo is not None:
            await Cache.update("lobbies", guild=guild_id, lobby=lobby_id, data={"negative_elo": negative_elo}, sqlcmds=[
                f"UPDATE lobbies SET negative_elo = {negative_elo} WHERE lobby_id = {lobby_id} AND guild_id = {guild_id}"
            ])

        # // Update queue size
        if queue_size is not None:
            await Cache.update("lobbies", guild=guild_id, lobby=lobby_id, data={"queue_size": queue_size}, sqlcmds=[
