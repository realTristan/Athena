    ...
    def __init__(self):
        with closing(db.cursor(buffered=True)) as cur:
            ...
            # // SETTINGS TABLE
                ...
                #cur.execute("CREATE TABLE settings (guild_id BIGINT, reg_role BIGINT, map_pick_phase VARCHAR(50), team_cap_vcs VARCHAR(50), picking_phase VARCHAR(50), queue_channel BIGINT, reg_channel BIGINT, win_elo int, loss_elo int, match_logs BIGINT, id int PRIMARY KEY AUTO_INCREMENT)")

            # // MATCHES TABLE
            #if not self.exists("SELECT *  FROM information_schema WHERE TABLE_NAME = matches"):
                #cur.execute("CREATE TABLE matches (guild_id BIGINT, match_id int, map VARCHAR(50), orange_cap VARCHAR(50), orange_team VARCHAR(50), blue_cap VARCHAR(50), blue_team VARCHAR(50), status VARCHAR(50), winners VARCHAR(50), id int PRIMARY KEY AUTO_INCREMENT)")
